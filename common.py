...
def make_retro(*, game, state=None, num_players, max_episode_steps=4500, **kwargs):
    ...
    env = retro.make(game, state, **kwargs, players=num_players, inttype=retro.data.Integrations.ALL, record='.')
...
def init_env(output_path, num_env, state, num_players, args, use_frameskip=True, use_display=False):
    ...
    def make_env(rank):
        def _thunk():
            env = make_retro(game=args.env, use_restricted_actions=retro.Actions.FILTERED, state=state,
...
def init_play_env(args):
    env = retro.make(game=args.env, state=args.state, use_restricted_actions=retro.Actions.FILTERED,
...
def init_model(output_path, player_model, player_alg, args, env):
    if player_alg == 'ppo2':
        ...
        if player_model == '':
            model = DQN(policy=args.nn, env=env,verbose=1, tensorboard_log=output_path)
...
def create_output_dir(args):
    output_dir = args.env + datetime.datetime.now().strftime('-%Y-%m-%d_%H-%M-%S.%f')
